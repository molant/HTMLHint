...
HTMLHint.addRule({
    ...
    init: function (parser, reporter, options) {
        ...
        if (typeof options === 'string') {
            ...
            if (!rule) {
                rule = {
                    'regId': new RegExp(options),
                    'message': 'The id and class attribute values do not meet your regex'
                };
            }
        }
        else {
    ...
    init: function(parser, reporter, options){
        ...
        parser.addListener('text', function(event){
            ...
            while((match = reMixed.exec(raw))){
                ...
                else if (options === 'tab' && /^\t+$/.test(match[2]) === false && match.input.trimRight().indexOf('\n') !== -1) {
